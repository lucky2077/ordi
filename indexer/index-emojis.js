...
async function processBatch(inscriptions) {
  const textInscriptions = inscriptions.filter(
    (it) => it.content_type === "text/plain;charset=utf-8"
  );
  ...
  for (const result of results) {
    ...
    //     it.num < storage.lowestEmojis[text].num
    //   ) {
    //     storage.lowestEmojis[text] = {
    //       id: it.id,
    //       num: it.num,
